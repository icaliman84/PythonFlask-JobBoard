...
def employer(employer_id):
    ...
    jobs = execute_sql('SELECT job.id, job.title, job.description, job.salary FROM job JOIN employer '
                       'ON employer.id = job.employer_id WHERE employer.id = ?', [employer_id], single=True)
    reviews = execute_sql('SELECT review, rating, title, date, status FROM review JOIN employer '
                          'ON employer.id = review.employer_id WHERE employer.id = ?', [employer_id], single=True)
